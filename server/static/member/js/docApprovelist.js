...
$(function() {
  $table1.bootstrapTable({
    ...
    uniqueId: "doc_id",   //每一行的唯一标识，一般为主键列
    classes:"table table-hover table-sm table-bordered text-center table-striped",
    theadClasses: "thead-dark",
    columns: [{field: 'doc_id', title: '醫生ID', visible: false},
              {field: 'member_id', title: '會員ID', formatter: idFormatter}, 
              {field: 'doc_name', title: '醫生姓名'}, 
              {field: 'clinic', title: '診所'}, 
              {field: 'title', title: '職稱'}, 
              {field: 'skill', title: '專長'}, 
  ...
  function idFormatter(value, row, index) {
    return [
      '<button type="button" class="showdoctor btn btn-sm btn-white border-0 text-blue"' +
      'data-id=' + row.doc_id ,
      ' ">' + value +'</button>'
    ].join('');
  ...
  $('#table1 tbody').on('click','tr td .showdoctor', function() {
    ...
    var doc_id = rowdata.doc_id;
    var url = "/member/docDetail?doc_id="+doc_id;
    location.href = url;
